...
1. **GitHub Pages source**, Settings → Pages → **Deploy from a branch** → **`gh-pages`** / **/(root)** (not `main`). If the site shows a blank page, open View Source: a broken deploy still has `<script src="/src/main.tsx">` instead of `/assets/index-*.js`.
